...
import { 
  ...
  ChevronRight, ArrowRight, ShieldCheck, FileText, Share2, Award
} from 'lucide-react';
...
export default function Footer({ locale = 'fr' }: { locale?: string }) {
  const isFR = locale === 'fr';
    ...
    },
    {
      ...
      icon: BookOpen,
      links: [
        { label: isFR ? 'Actualites & blog' : 'News & Blog', href: '/blog' },
  ...
  const stats = [
    {
      icon: Users,
      value: '960+',
      label: isFR ? 'Experts verifies' : 'Verified experts',
    },
    {
      icon: Building2,
      value: '25+',
      label: isFR ? 'Institutions listees' : 'Institutions listed',
    },
    {
      icon: Globe,
      value: 'FR / EN',
  ...
  const contactItems = [
    {
      icon: MapPin,
      title: isFR ? 'Siege' : 'Head office',
      value: 'Yaounde, Cameroun',
      href: withLocale('/contact'),
    },
    {
      icon: Phone,
      title: isFR ? 'Telephone' : 'Phone',
  ...
  const bottomItems = [
    ...
    {
      icon: FileText,
      label: isFR ? 'Propulse par FSPI & AFD' : 'Powered by FSPI & AFD',
    },
    {
      icon: Users,
      label: isFR ? 'Reseau professionnel verifie' : 'Verified professional network',
    },
  ];
        ...
        <div className="grid gap-14 lg:grid-cols-[1.08fr_1.62fr] lg:items-start lg:gap-16">
          <div className="space-y-10 lg:border-r lg:border-white/10 lg:pr-14">
            ...
            <p className="max-w-[560px] text-[20px] leading-[1.8] text-slate-300/95">
              {isFR
                ? "La plateforme nationale de reference concue pour structurer, valoriser et connecter l'expertise camerounaise dans les secteurs de l'eau et de l'assainissement."
